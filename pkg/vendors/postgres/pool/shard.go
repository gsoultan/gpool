...
// pool's policy, so it is passed in rather than duplicated here.
func (s *shard) takeIf(match func(*idleConn) bool) []*idleConn {
	if s.count.Load() == 0 {
		return nil
	}
